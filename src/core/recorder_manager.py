...
class VideoRecorder:
    ...
    def start_recording(self, duration: Optional[float] = None, 
        ...
        if enable_audio:
            ...
            self.logger.debug(f"Dispositivo: {audio_device_index}, Sample Rate: {sample_rate}, Canales: {audio_channels}")
            try:
                ...
                if not devices:
                    self.logger.warning("Audio habilitado pero no hay dispositivos disponibles. Continuando sin audio.")
                    self.audio_capture = None
                else:
                    self.audio_capture.start_capture(device_index=audio_device_index)
            except RuntimeError as e:
                self.logger.warning(f"No se pudo inicializar captura de audio: {e}. Continuando sin audio.")
                self.audio_capture = None
            except Exception as e:
                self.logger.error(f"Error inesperado al inicializar audio: {e}. Continuando sin audio.")
                self.audio_capture = None
    ...
    def _recording_loop(self):
        ...
        while self.is_recording:
            ...
            if current_time - last_frame_time >= self.frame_time:
                try:
                    if self.region:
                        ...
                        frame = self.screen_capture.capture_full_screen(self.monitor_index)
            ...
            if self.progress_callback:
                ...
                audio_mb = self.audio_capture.get_captured_size_mb() if self.audio_capture else 0.0
                self.progress_callback(elapsed, remaining, self.frames_captured, audio_mb)
            
            time.sleep(0.001)  # Pequeña pausa para no saturar CPU
    
    def _encoding_loop(self):
        """Loop de codificación."""
    ...
    def stop_recording(self) -> str:
        ...
        self.logger.warning("El codificador no estaba disponible al detener la grabación.")
        return None
    ...
    def get_elapsed_time(self) -> float:
        ...
        if self.is_paused and self.pause_start:
            return self.start_time - time.time() - self.paused_time - (time.time() - self.pause_start)
