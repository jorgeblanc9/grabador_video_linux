...
class MainWindow:
    ...
    def _create_recording_config(self, parent):
        ...
        tk.Label(duration_frame, text="h", bg=self.COLOR_BG, fg=self.COLOR_TEXT).pack(side=tk.LEFT)
        ...
        tk.Label(duration_frame, text="m", bg=self.COLOR_BG, fg=self.COLOR_TEXT).pack(side=tk.LEFT)
        ...
        tk.Label(duration_frame, text="s", bg=self.COLOR_BG, fg=self.COLOR_TEXT).pack(side=tk.LEFT)
        ...
        tk.Label(fps_frame, text="frames/segundo", bg=self.COLOR_BG, fg=self.COLOR_TEXT).pack(side=tk.LEFT)
    ...
    def _create_audio_config(self, parent):
        ...
        tk.Label(sample_frame, text="Hz", bg=self.COLOR_BG, fg=self.COLOR_TEXT).pack(side=tk.LEFT)
            ...
            if device_names:
                ...
                if not self.audio_device.get():
                    try:
                        ...
                        self.audio_device.set(f"{default_device['name']} ({default_device['index']})")
                    except RuntimeError:
                        # No hay dispositivos disponibles (común en WSL)
                        self.audio_device_combo['values'] = ["No hay dispositivos disponibles"]
                        self.logger.info("No se encontraron dispositivos de audio (normal en WSL)")
            else:
    ...
    def _start_recording(self):
        ...
        try:
            ...
            if not validate_output_path(self.output_file.get()):
                messagebox.showerror("Error", "La ruta de salida no es válida o no tiene permisos de escritura")
                return
            ...
            if total_seconds == 0:
                self.logger.debug("Duración no especificada, grabando sin límite de tiempo.")
                total_seconds = None  # Sin límite de tiempo
            else:
                self.logger.debug(f"Duración total de la grabación: {total_seconds} segundos.")
            ...
            if not self.full_screen.get() and self.region_selected:
                ...
                self.logger.debug(f"Grabando región seleccionada: {self.region_selected}")
            else:
            ...
            if enable_audio:
                ...
                try:
                    ...
                    if not devices:
                        self.logger.warning("Audio habilitado pero no se encontraron dispositivos.")
                        response = messagebox.askyesno(
                        ...
                        if not response:
                            return
                        enable_audio = False
                    elif self.audio_device.get() and "No hay" not in self.audio_device.get():
                        self.logger.debug(f"Dispositivo de audio seleccionado: {self.audio_device.get()}")
                    ...
                    self.logger.warning(f"Error al verificar dispositivos de audio: {e}")
                    enable_audio = False
            else:
                self.logger.info("Audio deshabilitado por el usuario.")
            
            channels = 2 if self.audio_channels.get() == "Stereo" else 1
            self.logger.debug(f"Configuración de audio: Sample Rate={self.sample_rate.get()}, Canales={channels}")
    ...
    def _stop_recording(self):
        ...
        try:
            ...
            if output_file:
                ...
                messagebox.showwarning("Advertencia", "La grabación se detuvo pero no se generó archivo")
